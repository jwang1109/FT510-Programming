...
int countMines(board_t * b, int x, int y) {
  ...
  int count =0 ;
  if(x==0){
    if(IS_MINE(b->board[y][x+1])){count+=1;}
    if(y==0){//left down corner
      if(IS_MINE(b->board[y+1][x])){count+=1;}
      if(IS_MINE(b->board[y+1][x+1])){count+=1;}
    }
    else if(y==((b->height)-1)){//left up corner
      if(IS_MINE(b->board[y-1][x])){count+=1;}
      if(IS_MINE(b->board[y-1][x+1])){count+=1;}
    }
    else{//left boader
	...
	if(IS_MINE(b->board[y+1][x])){count+=1;}
      ...
      else if (y==b->height -1){//right up corner
	if(IS_MINE(b->board[y-1][x-1])){count+=1;}
	if(IS_MINE(b->board[y-1][x])){count+=1;}
      }
      else{//right boader
    ...
    if(IS_MINE(b->board[y][x+1])){count+=1;}
    if(IS_MINE(b->board[y][x-1])){count+=1;}
    if(y==0){//bottom boader
      if(IS_MINE(b->board[y+1][x])){count+=1;}
      if(IS_MINE(b->board[y+1][x+1])){count+=1;}
      if(IS_MINE(b->board[y+1][x-1])){count+=1;}
    }
    else if (y==(b->height-1)){//up boader
      if(IS_MINE(b->board[y-1][x+1])){count+=1;}
      if(IS_MINE(b->board[y-1][x])){count+=1;}
      if(IS_MINE(b->board[y-1][x-1])){count+=1;}
    }
